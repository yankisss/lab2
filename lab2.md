...
> Первая строка: целое число a0 — нулевой член последовательности.
> Вторая строка: n натуральных целых чисел — количество элементов последовательности.

| Переменная | Тип               | min    | max  |
| ---------- | ----------------- | ------ | ---- |
| a0         | Целое число       | -10000 | 10000|
...
| Переменная | Тип               | min               | max |
| ---------- | ----------------- | ----------------- | --- |
| a_1..a_n    | Натуральное число | зависят от a0 и n | —   |
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        // Проверяем, что m положительное
        if (m <= 0) {
            out.print(0);
            return;
        }
        
        // Проверяем, что x в пределах [0, m-1]
        // Если x отрицательное или больше m, корректируем
        x = ((x % m) + m) % m;
        
        int[] visited = new int[m];
        int step = 0;
        
        while (x >= 0 && x < m && visited[x] == 0) {
            visited[x] = step + 1;
            x = (a * x + c) % m;
            step++;
        }
